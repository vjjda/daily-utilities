...
"""
Logic thực thi/hành động cho pack_code.
...
def execute_pack_code_action(logger: logging.Logger, result: Dict[str, Any]) -> None:
    ...
    if status == 'empty':
        ...
        logger.error(f"Logic cốt lõi thất bại: {result.get('message', 'Lỗi không xác định')}") #
        sys.exit(1)
    elif status != 'ok':
        logger.error("Logic cốt lõi thất bại với trạng thái không xác định, executor bị hủy.") #
        sys.exit(1)
