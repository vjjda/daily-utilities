...
def is_path_matched(
    ...
    try:
        # Sử dụng os.path.relpath để lấy đường dẫn tương đối
        # của chính file/symlink mà không bám theo (resolve) nó.
        # Chúng ta resolve() start_dir để đảm bảo nó là tuyệt đối.
        resolved_start_dir = start_dir.resolve()
        
        # Lấy đường dẫn của file (có thể là symlink)
        # và resolve() nó, nhưng KHÔNG bám theo (strict=False)
        # Note: strict=False là mặc định trên Python < 3.10, 
        # nhưng an toàn hơn khi làm tường minh (mặc dù Path().resolve() không có strict=False)
        # Chúng ta sẽ dùng os.path.relpath
        
        # Cách an toàn nhất: không resolve() path, chỉ resolve() start_dir
        # và để pathspec xử lý.
        
        # SỬA: Dùng os.path.relpath để xử lý symlink an toàn
        relative_path_str = os.path.relpath(path.as_posix(), resolved_start_dir.as_posix())
